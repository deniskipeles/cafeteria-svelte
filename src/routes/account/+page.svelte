<script>
    ...
    if ($user.id == null || $user.id == undefined) {
        let result = url.includes("/account");
      if (result) {
...
<div class="flex flex-col justify-center max-w-xs p-6 shadow-md rounded-xl sm:px-12 dark:bg-gray-900 dark:text-gray-100">
	{#if $user.profile}
            <!-- content here -->
            <img src={`${url}/api/files/${$user.profile["@collectionId"]}/${$user.profile.id}/${$user.profile.avatar}?thumb=200x200`} alt="" class="w-32 h-32 mx-auto rounded-full dark:bg-gray-500 aspect-square">
            {:else}
            <!-- else content here -->
            <img src="https://upload.wikimedia.org/wikipedia/commons/c/cb/Student_icon.svg" alt="" class="w-32 h-32 mx-auto rounded-full dark:bg-gray-500 aspect-square">
    {/if}
    
	<div class="space-y-4 text-center divide-y divide-gray-700">
		<div class="my-2 space-y-1">
			<h2 class="text-xl font-semibold sm:text-2xl">{$user.profile ? $user.profile.name : "No name yet"}</h2>
